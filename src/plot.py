...
def formatlines(data, isGradient, indices, intervals, normalize, n):
    print("formatlines.n,shape(data) = " + str((n, shape(data[n]))))
    if len(indices):
        if isGradient:
...
def plot_radar(args):
    ...
    (data, vmap) = (groupby(len(files), data), {'VM': [label(f) for f in files]})
    ...
    # Ind1
    values=df.loc[0].drop('VM').values.flatten().tolist()
    values += values[:1]
    ax.plot(angles, values, linewidth=1, linestyle='solid', label="group A")
    ax.fill(angles, values, 'b', alpha=0.1)
 
    # Ind2
    values=df.loc[1].drop('VM').values.flatten().tolist()
    values += values[:1]
...
def plot_hist(args):
    ...
    for c in range(len(data[0][0] if len(data) else [])):
        ...
        lbl = get(nextv(args, '--legend', 'Metric').split("|"), c)
        rect = bar(args, ax, ind-(c*width), mean, width, std, lbl)
    lblticks(args, ax, ind, [label(f) for f in files])
...
def bar(args, ax, pos, vals, width, std, lbl):
    if "--rotate" in args:
        ax.barh(pos, vals, width, xerr=std, label=lbl)
    else:
        ax.bar(pos, vals, width, yerr=std, label=lbl)


def lblticks(args, ax, ind, labels):
    if "--rotate" in args:
        ax.set_xlabel(nextv(args, '--yaxis', 'Value'))
        ax.set_yticks(ind)
...
def label(fname):
    f = fname
    for t in ['predictive', 'steps']:
        f = f.replace(t, '')
    f = re.sub('[0-9]+\.[0-9]+\..*\.pickle','', f)
    f = f[0:2]+'.'+f[2:] if not f[2]=='.' else f
    return f
...
def usage():
    print("\nUsage: " + sys.argv[0] + " [-h | -s | -l | -p | -3] [file(s)]\n" +
        ...
        "--hist            TEXT  Histogram plot\n" +
        "--xaxis           TEXT  x-axis label (with --line)\n" +
        "--yaxis           TEXT  y-axis label (with --line)\n" +
        "--title           TEXT  title (with --line)\n" +
        "--legend          TEXT  legend (with --hist)\n" +
        "--rotate          TEXT  horizontal histogram (with --hist)\n" +
...
def main():
    try:
        opts, args = getopt(sys.argv[1:], "hslpatr:3", ["help", "scatter", "line", "predictions", "surface3d", "all", "multi", "hist", "radar"])
